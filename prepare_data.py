...
def get_data(pic_shape, leng=100000, augmentation=False, gray=False):
    ...
    multi_list = [1]*7
    if augmentation:
        multi_list = [min(max(int(5000/len(metadata[metadata["dx_id"] == i])),1),20)for i in range(7)]
        print(multi_list)
    ...
    class_weights_train = {i: len(train) / (len(train[train["dx_id"] == i])*7) for i in range(7)}
    class_weights_test = {i: len(test) / (len(test[test["dx_id"] == i])*7) for i in range(7)}
    ...
    class_weights_test = {0: len(test)/(len(test[test["dx_id"] == 1])*2), 1: len(test)/(len(test[test["dx_id"] == 3])*2)}
    class_weights_train = {0: len(train) / (len(train[train["dx_id"] == 1])*2), 1: len(train) / (len(train[train["dx_id"] == 3])*2)}
    print("test", class_weights_test)
    print("train", class_weights_train)
